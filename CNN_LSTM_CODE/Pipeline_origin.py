...
#         self.shared_output = tf.keras.layers.Dense(2, activation='sigmoid', name='shared_output')  

#     def call(self, metrics):  # Now taking metrics as input, not predictions
#         metrics = tf.reshape(metrics, (-1, 3))  # Reshape to (batch_size, 3)
#         x = self.dense1(metrics)
#         # x = self.dense2(x)
#         # x = self.dense3(x)
#         shared_output = self.shared_output(x)  # Output shape: (batch_size, 2)

#         # Split into alpha and gamma
#         alpha, gamma = tf.split(shared_output, num_or_size_splits=2, axis=-1)

#         return alpha, gamma


    
# def dynamic_focal_loss(meta_learner):
#     """
#     Binary form of focal loss with dynamic alpha and gamma from MetaLearner.
#     The alpha and gamma are computed using validation metrics instead of y_pred.
#     """
#     def focal_loss_fixed(y_true, y_pred):
#         y_true = tf.cast(y_true, tf.float32)

#         # Ensure predictions are within the [epsilon, 1-epsilon] range
#         y_pred = tf.clip_by_value(y_pred, tf.keras.backend.epsilon(), 1.0 - tf.keras.backend.epsilon())

#         # Calculate evaluation metrics like F1, Precision, Recall (use a validation loop)
#         metrics = calculate_metrics(y_true, y_pred)

#         # Get alpha and gamma from the MetaLearner based on the metrics
#         alpha, gamma = meta_learner(metrics)

#         # Compute alpha_t
#         alpha_t = y_true * alpha + (tf.ones_like(y_true) - y_true) * (1 - alpha)

#         # Compute p_t (probability for true class)
...
#         return tf.reduce_mean(focal_loss)  # Reduce mean across the batch
...
#     """
#     Compute Precision, Recall, and F1 score using TensorFlow operations.
#     """
#     # Ensure predictions are binary by rounding the predictions
...
#     # Return metrics as a TensorFlow tensor
#     return tf.stack([f1, precision, recall], axis=0)
